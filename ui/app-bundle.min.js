 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */};class ut extends ${constructor(){super(...arguments),this.renderOptions={host:this},this.ht=void 0}createRenderRoot(){const t=super.createRenderRoot();return this.renderOptions.renderBefore??=t.firstChild,t}update(t){const s=this.render();this.hasUpdated||(this.renderOptions.isConnected=this.isConnected),super.update(t),this.ht=lt(s,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this.ht?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this.ht?.setConnected(!1)}render(){return Z}}ut._$litElement$=!0,ut["finalized"]=!0,globalThis.litElementHydrateSupport?.({LitElement:ut});const dt=globalThis.litElementPolyfillSupport;dt?.({LitElement:ut});(globalThis.litElementVersions??=[]).push("4.0.5");class TypesNav extends ut{static properties={types:{type:Array}};async connectedCallback(){super.connectedCallback();this.types=await api.types()}path(t){return`/type/${t}`}render(){if(!this.types)return"";return q`
      <nav class="subnav">
        ${this.types.map((type=>q`
            <a href="${this.path(type._type)}">${type._type} (${type.total})</a>
          `))}
      </nav>
    `}createRenderRoot(){return this}}customElements.define("types-nav",TypesNav);class TypesIndex extends ut{static properties={type:{},items:{type:Array}};async connectedCallback(){super.connectedCallback();let res=await api.type_index(this.type);this.items=res?.rows}path(el){if(this.type=="__paths")return`/id/${encodeURIComponent(el.id)}`;return`/id/${encodeURIComponent(el._id)}`}title(el){if(this.type=="__paths")return el.path;let title=el.title;if(!title)title=el.name;if(!title)title=el._id;return title}render(){console.log("render type index????",this.items);if(!this.items)return"";console.log("render type index",this.items);return q`
      <h1>${this.type=="__paths"?"paths":this.type}</h1>
      <ol>
        ${this.items.map((el=>q` <li><a href=${this.path(el)}>${this.title(el)}</a></li>`))}
      </ol>
    `}createRenderRoot(){return this}}customElements.define("types-index",TypesIndex);class DocumentView extends ut{static properties={docid:{},doc:{type:Object}};async connectedCallback(){super.connectedCallback();let res=await api.document(this.docid);console.log("document",res);this.doc=res}path(el){if(this.type=="__paths")return`/id/${encodeURIComponent(el.id)}`;return`/id/${encodeURIComponent(el._id)}`}title(el){if(this.type=="__paths")return el.path;let title=el.title;if(!title)title=el.name;if(!title)title=el._id;return title}render(){console.log("render type index????",this.items);if(!this.doc)return"";console.log("render type index",this.items);return q`
      <h1>${this.doc._id}</h1>
      <json-explorer .data=${this.doc}></json-explorer>
    `}}customElements.define("document-view",DocumentView);let tmpl=document.createElement("template");tmpl.innerHTML=`\n<section>\n<div id="editor" contenteditable="true"></div>\n<div id="control"><button class="play">play</button></div>\n</section>\n<div id="results">\n<json-explorer></json-explorer>\n</div>\n\n<style>\nsection{\n  position:relative;\n  width:65ch;\n  background-color:#fafafa;\n  border: 2px solid #e6e6e6\n}\n#editor{\n      min-height: 5rem;\n    padding: 0.5rem 1rem 2rem 0.5rem;\n    white-space: pre-wrap;\n    font-family: "SFMono-Regular", Consolas, "Liberation Mono", Menlo, Courier, monospace;\n    color:black;\n  }\n#editor:empty::before {\n    display: inline-block;\n    content: '*(_type=="...")';\n    color: #999;\n}\n#control{\n  position:absolute;\n  bottom:0;\n}\nbutton{\n  border:none;\n  background-color:transparent;\n  font-size:1.2rem;\n  padding:0.5rem;\n}\nbutton:hover{\n  background-color:var(--cgelb);\n}\n  #results{\n  margin-top: 1rem;\n  }\n</style>\n`;class LolqlPlayground extends HTMLElement{constructor(){super().attachShadow({mode:"open"}).appendChild(tmpl.content.cloneNode(true));this.editor=this.shadowRoot.querySelector("#editor");this.play=this.shadowRoot.querySelector(".play");this.results=this.shadowRoot.querySelector("json-explorer");this.play.addEventListener("click",this)}get query(){const selectedQuery=window.getSelection().toString().trim();return selectedQuery||this.editor.innerText}async handleEvent(e){console.log("++ exec",this.query);let res=await api.lolql(this.query);this.results.data={result:res}}}customElements.define("lolql-playground",LolqlPlayground);let main=document.querySelector("main");let refetch_btn=document.querySelector("#refetch");const router=NavigoPlus("/__ui");router.on("/type/:type",(({data:data})=>{main.innerHTML=`<types-index type="${data.type}"></types-index>`})).on("/id/:id",(({data:data})=>{main.innerHTML=`<document-view docid="${data.id}"></document-view>`})).on("/playground",(()=>{main.innerHTML=`<h1>lolql playground</h1>\n    <lolql-playground></lolql-playground>`})).on("/about",(()=>{main.innerHTML=`<h1>slowfoot navigator</h1>\n    <p>navigate through your data</p>`})).on("/",(()=>{main.innerHTML=``}));refetch_btn.addEventListener("click",(()=>{refetch_btn.disabled=true;api.refetch().finally((()=>refetch_btn.disabled=false))}));router.start_from_current_url();