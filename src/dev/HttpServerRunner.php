...
class HttpServerRunner {
    ...
    public function __construct() { // LogStreamHandler $logger, ?string $listenAddress
        ...
        $this->listenAddress = $_ENV["X_LISTEN"]; // $listenAddress ?? '127.0.0.1:8080';
    }

    public function stop() {
        $logger = $this->logger;
        $socket = $this->socket;
        $logger->log('restarting server, stopping loop');
        $socket->close();
        unset($this->http);
        Loop::stop();
    }
    ...
    public function __invoke(callable $handler): void {
        $this->socket = new SocketServer($this->listenAddress);
        $socket = $this->socket;

        // create HTTP server, automatically start new fiber for each request on PHP 8.1+
        $http = new HttpServer(...(\PHP_VERSION_ID >= 90100 ? [new FiberHandler(), $handler] : [$handler]));
        // $http = $this->http;
        ...
        try {
            Loop::addSignal(\defined('SIGINT') ? \SIGINT : 2, $f1 = static function () use ($socket, $logger, $http): void {
            ...
            Loop::addSignal(\defined('SIGTERM') ? \SIGTERM : 15, $f2 = static function () use ($socket, $logger, $http): void {
